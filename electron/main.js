...
function waitForServer(url, timeout = 3e4) {
  return new Promise((resolve, reject) => {
    const start = Date.now();
    const parsed = new URL(url);
    const check = () => {
      const req = http.get(
        { hostname: parsed.hostname, port: parsed.port, path: parsed.pathname, timeout: 2e3 },
        (res) => {
...
async function startServer() {
  ...
  if (isDev) {
    ...
    try {
      await waitForServer(devUrl, 1e3);
      ...
      await waitForServer(devUrl, 3e4);
